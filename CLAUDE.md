...
Reminder notifications are produced by `createDueReminderNotifications()`, a claim-then-notify sweep (via `updateMany` with `reminderNotifiedAt: null` as an optimistic lock). It runs primarily from a Vercel Cron job (`crons` in `vercel.json` → `POST/GET /api/cron/reminders`, guarded by `CRON_SECRET`, every minute). `GET /notifications` still runs it as a fallback, but throttled to at most once per `REMINDER_SWEEP_MIN_INTERVAL_MS` (90s) per warm instance via the module-level `lastReminderSweepAt` timestamp — so most notification fetches are just two indexed queries.
