...
PostgreSQL via Prisma. Core entities: `UserProfile` (app-level user, distinct from the Google identity), `Workspace`, `WorkspaceMember` (join table with `role`), `Task` (belongs to a `Workspace`), `TaskComment`, `Invitation` (token-based, email-addressed, with expiry), `Notification` (comment / assignment / status / reminder types; comment + reminder go to workspace OWNERs plus the task's assignee, not all members). Local dev DB is SQLite (`backend/prisma/dev.db`) per some scripts, but the schema's configured provider is `postgresql` — check `DATABASE_URL` before assuming which is active.

Production Postgres is Neon. `schema.prisma` has both `url = env("DATABASE_URL")` (pooled `-pooler` endpoint, used by `PrismaClient` at runtime) and `directUrl = env("DIRECT_URL")` (non-pooled endpoint, used by Prisma CLI — `migrate deploy` runs on every Vercel build via `backend/scripts/postinstall.cjs` and would hang on the pooler). Both env vars must be set wherever Prisma runs (Vercel: both; local `backend/.env`: both, same localhost value).

Reminder notifications are produced by `createDueReminderNotifications()`, a claim-then-notify sweep (via `updateMany` with `reminderNotifiedAt: null` as an optimistic lock). `GET /notifications` triggers it, but throttled to at most once per `REMINDER_SWEEP_MIN_INTERVAL_MS` (60s) per warm instance via the module-level `lastReminderSweepAt` timestamp — so most notification fetches are just two indexed queries. There is also a standalone `app.all(["/cron/reminders", "/api/cron/reminders"])` endpoint (guarded by the `CRON_SECRET` env var, not `authenticate`) that runs the same sweep and returns a count — wire an external scheduler to it for user-independent timing. (Vercel's own `crons` in `vercel.json` is **not** used: it's incompatible with this repo's legacy `builds` config.)
